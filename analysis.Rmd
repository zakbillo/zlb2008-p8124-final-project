...
Run Lasso for the MAP3K5−AS2 neighborhood and compare lambda seq results against lambda.1se to make a tractable network of the top 25 most stable genes
...
estimate_downstream_effects_safe_parallel <- function(data, cause_var, n_boots = 100, file_name = NULL) {
  ...
  # Setup 
  alpha_val <- 0.01  
  node_names <- colnames(data) # Setting nodes
  cause_idx <- which(node_names == cause_var) # Calling index of cause variable
  
  if (length(cause_idx) == 0) stop("Cause variable not found in data.") # Fail-safe for cause variable loading
  
  potential_targets <- node_names[-cause_idx] # Targets are everything but the cause var
  target_indices <- setdiff(seq_along(node_names), cause_idx) 
  n_targets <- length(target_indices) # Count number of target rows
  n_obs <- nrow(data) # Count number of observations in dataset
  csv_name <- if(is.null(file_name))
  ...
  # creating a progress bar to track how far along we have gotten 
  pb <- txtProgressBar(max = n_boots, style = 3)
  opts <- list(progress = function(n) setTxtProgressBar(pb, n))
  
  # Export workspace variables 
  vars_to_export <- c("data", "cause_idx", "alpha_val", "node_names", "target_indices", "n_targets", "n_obs")
  ...
  message(paste("Processing", n_boots, "bootstraps on", n_cores, "cores..."))
  
  # Parallel bootstrap Loop
  boot_results <- foreach( 
    b = 1:n_boots, 
    .packages = c("pcalg", "graph", "methods"), # Packages for each worker
    .export = vars_to_export, 
    .options.snow = opts # Show progress 
  ) %dopar% {
    ...
    mcov <- cov(boot_data, use = "pairwise.complete.obs") # Cov matrix 
    
    # Estimate PAG
    pag_est <- tryCatch({
      rfci(suffStat, indepTest = gaussCItest, alpha = alpha_val, 
           labels = node_names, verbose = FALSE, m.max = 4)
    }, error = function(e) return(NULL)) # Skip is estimation fails
    
    if (is.null(pag_est)) return(NULL) # Skip if RFCI fails
    
    amat <- pag_est@amat # Get amat
    
    is_cyclic <- is.cyclic(amat)
    
    if (is_cyclic) return(NULL)
    
    # Calculating causal effects
    row_eff <- numeric(n_targets) # Matrix for row effects
    descendants <- pcalg::possibleDe(amat, cause_idx)
    
    # Only check nodes reachable from the cause
    ...
    if (length(active_targets) > 0) {
      for (target_idx in active_targets) {
        ...
        effs <- lv.ida(cause_idx, target_idx, mcov, amat, method = "local")
        
        # Take the median for bootstrap
...
# Run
resultsasthma <- estimate_downstream_effects_safe_parallel(X_asthma_stable, "MAP3K5-AS2", n_boots = 100, "asthma lvida.csv")
resultsnorm <- estimate_downstream_effects_safe_parallel(X_normal_stable, "MAP3K5-AS2", n_boots = 100, "normal lvida.csv")
resultsfull <- estimate_downstream_effects_safe_parallel(X_filtered_stable, "MAP3K5-AS2", n_boots = 100, "overall ida.csv")
```
...
final_table <- merged_df |>
  select(Target_Gene, Median_Effect_Asthma_Str, Median_Effect_Normal_Str) |>
  rename(
    Median_Effect_Asthma = Median_Effect_Asthma_Str,
    Median_Effect_Normal = Median_Effect_Normal_Str
  )

write.csv(final_table, "asthma_vs_normal.csv", row.names = FALSE)
```

Make that table nicer

```{r}
nicetable <- final_table |>
  gt() |>
  
  # Title
  tab_header(
    title = md("**Differential Effect Analysis**"),
    subtitle = "Comparison of Median Effects between Asthma and Normal Groups"
  ) |>
  
  # Rename
  cols_label(
    Target_Gene = "Target Gene",
    Median_Effect_Asthma = "Asthma Group",
    Median_Effect_Normal = "Normal Group"
  ) |>
  
  # Align
  cols_align(
    align = "center",
    columns = c(Median_Effect_Asthma, Median_Effect_Normal)
  ) |>
  
  # Footnotes and style 
  tab_footnote(
    footnote = "Values displayed as Median (95% Confidence Interval).",
    locations = cells_column_labels(columns = c(Median_Effect_Asthma, Median_Effect_Normal))
  ) |>

  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(columns = Target_Gene)
  )

# Save
print(nicetable)

gtsave(nicetable, "nicetable.png")
```
